...
Libraries
```{r}
# KG added this section, and moved all library calls to the beginning of the script
library(RSQLite)
library(here)
library(tidyverse)
library(lme4)
...
fm <- dbTable("fish_morphometrics") # pull in fish morphometrics data
```
...
# KG: transform the eye width data from the FISH_MORPHOMETRICS table so that it's in exactly the same format as eyeWidthDat.
ew <- fm %>%
  filter(parameter == "eyeWidth")
all(ew$parameterValue == eyeWidthDat$eyeWidth) # parameters match up.

ew <- ew %>%
  rename("imageID" = imageFile,
         "eyeWidth" = parameterValue) %>%
  select(-parameter) %>%
  mutate(captureMethod = str_extract(fishID, "(?<=\\d{3,4}\\_)[A-Z]+(?=\\_)")) %>%
  mutate(captureMethod = fct_recode(captureMethod,
                                    "Electrofishing" = "DE",
                                    "Angling" = "AN",
                                    "Fyke_Net" = "FN"),
         lakeID = str_extract(fishID, "^[A-Z]+(?=\\_)"))
...
finAngleData<-read.csv(here("data", "unclassified", "Pec Fin Angles_ORIGINAL.csv"))
# XXX from here, going to check out a new branch called "pecFinAngles" and proceed forward assuming that Angle.pos is equivalent to "ang_deg"
...
# angle.pos<-finAngleData$Angle.pos # can ignore all the stuff with angle.pos--that's all old.
